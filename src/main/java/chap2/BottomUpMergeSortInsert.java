...
public class BottomUpMergeSortInsert<T> implements Sortable<T>
{
	...
	// Do insertion sort by CUTOFF 
	private void preHandle(Comparable<T>[] a)
	{
		for(int i = 0; i < a.length; i += CUTOFF)
		{
			InsertionSort.sort(a, i, Math.min(i + CUTOFF - 1, a.length - 1));
		}
