...
//		} 
		...
		for(int i = 0; i < test.length; i++) {
			...
			// 检查test[i]是否已经出现在堆中了
			if(null == map.get(test[i]) || false == map.get(test[i])) {
				// 若没有出现过才添加
		...
		for(int i = currentIndex; i < test.length; i++) {
			...
			// 注意这里条件运算的结合顺序
			if(test[i] > heap[0] && (null == map.get(test[i]) || false == map.get(test[i]))){
				// 将当前堆顶的元素在map中的对应位置置false
				map.put(heap[0], false);
				heap[0] = test[i];
				map.put(test[i], true);
				sink(heap, 0, k);
			}
	...
	public void testTopK() {
		...
		for(int k : TopKWithoutDups.process(test, 3)) {
